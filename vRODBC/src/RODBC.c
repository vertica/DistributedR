...
/*
* Change Log - vRODBC
* Code added to handle Vertica ODBC driver specific- return data types such as 
* SQL_FLOAT, SQL_BIGINT, SQL_NUMERIC and SQL_BIT for floating point numbers, Integers and 
* Boolean respectively.
*
* Date          Name
* ------------------------
* 08-15-2013   HP Vertica
*/
...
typedef struct cols {
    ...
    SQLBIGINT   I3Data[MAX_ROWS_FETCH];
...
static char 
err_SQLAllocStmt[]=gettext_noop("[vRODBC] ERROR: Could not SQLAllocStmt");
...
static void odbcInit(void)
{
	...
	if(retval == SQL_SUCCESS || retval == SQL_SUCCESS_WITH_INFO) {
	    ...
	    error(_("[vRODBC] ERROR: Could not SQLAllocEnv"));
    }
}
...
SEXP RODBCDriverConnect(SEXP connection, SEXP id, SEXP useNRows, SEXP ReadOnly)
{
	...
	warning(_("[vRODBC] ERROR:invalid connection argument"));
	UNPROTECT(1);
	return ans;
    }
	...
	if(retval == SQL_SUCCESS || retval == SQL_SUCCESS_WITH_INFO) {
		...
		while(1) {
		    ...
		    warning(_("[vRODBC] ERROR: state %s, code %d, message %s"),
			    state, code, msg);
		}
	    } else warning(_("[vRODBC] ERROR: Could not SQLDriverConnect"));
	    (void)SQLFreeHandle(SQL_HANDLE_DBC, thisHandle->hDbc);
	}
    } else {
	warning(_("[vRODBC] ERROR: Could not SQLAllocConnect"));
    }
    UNPROTECT(1);
    return ans;
}
...
SEXP RODBCQuery(SEXP chan, SEXP query, SEXP sRows)
{
	...
	if( res != SQL_SUCCESS && res != SQL_SUCCESS_WITH_INFO ) {
	    ...
	    sprintf(message,
		    "[vRODBC] ERROR: Could not SQLExecDirect '%s'", cquery);
	    geterr(thisHandle);
...
SEXP RODBCPrimaryKeys(SEXP chan, SEXP table, SEXP cat, SEXP schem)
{
	...
	res = SQLPrimaryKeys( thisHandle->hStmt,
			      ...
			      SQL_NTS);
	success_or_failure(_("[vRODBC] ERROR: Failure in SQLPrimary keys"));
    }
    return ScalarInteger(stat);
}
...
SEXP RODBCColumns(SEXP chan, SEXP table, SEXP cat, SEXP schem, SEXP sLiteral)
{
	...
	res = SQLColumns( thisHandle->hStmt,
			  ...
			  (SQLCHAR *) translateChar(STRING_ELT(table, 0)),
			  SQL_NTS, NULL, 0);
	success_or_failure(_("[vRODBC] ERROR: Failure in SQLColumns"));
    }
    return ScalarInteger(stat);
}
...
SEXP RODBCSpecialColumns(SEXP chan, SEXP table, SEXP cat, SEXP schem)
{
	...
	success_or_failure(_("[vRODBC] ERROR: Failure in SQLSpecialColumns"));
    }
    return ScalarInteger(stat);
}
		 ...
		 SEXP sLiteral)
{
	...
	res = SQLTables(thisHandle->hStmt,
			...
			(SQLCHAR *) tName, len3,
			(SQLCHAR *) tType, len4);
	success_or_failure(_("[vRODBC] ERROR: SQLTables failed"));
    }
    return ScalarInteger(stat);
}
...
static int cachenbind(pRODBCHandle thisHandle, int nRows)
{
    ...
    if( retval != SQL_SUCCESS && retval != SQL_SUCCESS_WITH_INFO ) {
	geterr(thisHandle);
	errlistAppend(thisHandle, _("[vRODBC] ERROR: SQLRowCount failed"));
	goto error;
    }
	...
	if( retval != SQL_SUCCESS && retval != SQL_SUCCESS_WITH_INFO ) {
	    ...
	    errlistAppend(thisHandle, 
			  _("[vRODBC] ERROR: SQLDescribeCol failed"));
	    goto error;
	}
	...
	case SQL_DOUBLE:
        ...
        case SQL_BIGINT:
            BIND(SQL_C_SBIGINT, I3Data, sizeof(long));
	case SQL_BIT:
        case SQL_INTEGER:
	...
	if( retval != SQL_SUCCESS && retval != SQL_SUCCESS_WITH_INFO ) {
	    geterr(thisHandle);
	    errlistAppend(thisHandle, _("[vRODBC] ERROR: SQLBindCol failed"));
	    goto error;
	}
    }
    return 1;
...
SEXP RODBCNumCols(SEXP chan)
{
    ...
    if(NCOLS == -1)
	errlistAppend(thisHandle, _("[vRODBC] No results available"));
	...
	for(j = 1; j <= maximum; ) {
		...
		for (i = 0; i < nc; i++) {
			...
			break;
            		...
            		REAL(VECTOR_ELT(data, i))[j-1] =
                 	    len == SQL_NULL_DATA ? NA_REAL :
                 	    thisHandle->ColData[i].I3Data[row];
            		break;
                    case SQL_BIT:
                    case SQL_INTEGER:
...
SEXP RODBCColData(SEXP chan)
{
	...
	case SQL_BIGINT: SET_STRING_ELT(type, i, mkChar("bigint")); break;
        case SQL_SMALLINT: SET_STRING_ELT(type, i, mkChar("smallint")); break;
	case SQL_FLOAT: SET_STRING_ELT(type, i, mkChar("double")); break;
		 ...
		 SEXP params, SEXP test)
{
	...
	char *message = Calloc(strlen(cquery)+50, char);
	sprintf(message,
		"[vRODBC] ERROR: Could not SQLPrepare '%s'", cquery);
	geterr(thisHandle);
	errlistAppend(thisHandle, message);
	(void)SQLFreeHandle(SQL_HANDLE_STMT, thisHandle->hStmt);
	thisHandle->hStmt = NULL;
	...
	if(res  != SQL_SUCCESS && res != SQL_SUCCESS_WITH_INFO) {
	    geterr(thisHandle);
	    errlistAppend(thisHandle, _("[vRODBC] SQLBindParameter failed"));
	    geterr(thisHandle);
	    stat = -1;
	    goto end;
	}
	...
	if(vtest < 2) {
	    ...
	    if( res != SQL_SUCCESS && res != SQL_SUCCESS_WITH_INFO ) {
		errlistAppend(thisHandle, _("[vRODBC] Failed exec in Update"));
		geterr(thisHandle);
		stat = -1;
		goto end;
	    }
...
static int inRODBCClose(pRODBCHandle thisHandle)
{
	...
	warning(_("[vRODBC] Error in SQLDisconnect"));
	success = -1;
    }
    retval = SQLFreeHandle(SQL_HANDLE_DBC, thisHandle->hDbc);
    if( retval != SQL_SUCCESS && retval != SQL_SUCCESS_WITH_INFO ) {
	warning(_("[vRODBC] Error in SQLFreeconnect"));
	success = -1;
    }
    cachenbind_free(thisHandle);
    errorFree(thisHandle->msglist);
...
static void chanFinalizer(SEXP ptr)
{
    ...
    warning(_("closing unused vRODBC handle %d\n"),
...
void R_init_vRODBC(DllInfo *dll)
{
