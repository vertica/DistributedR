...
.onUnload <- function(libpath)
{
    odbcCloseAll()
    .Call(C_RODBCTerm)
    library.dynam.unload("vRODBC", libpath)
}

odbcGetErrMsg <- function(channel)
{
    if(!odbcValidChannel(channel))
       stop("first argument is not an open vRODBC channel")
...
{
    if(!inherits(channel, "vRODBC"))
        stop("Argument 'channel' must inherit from class vRODBC")
              ...
              DBMSencoding= "", rows_at_time = 100, readOnlyOptimize = FALSE)
{
   ...
   structure(stat, class = "vRODBC", case = case, id = id,
...
close.vRODBC <- function(con, ...) odbcClose(con)

odbcClose <- function(channel)
{
    if(!odbcValidChannel(channel))
       stop("argument is not an open vRODBC channel")
...
{
    inherits(channel, "vRODBC") && is.integer(channel) &&
