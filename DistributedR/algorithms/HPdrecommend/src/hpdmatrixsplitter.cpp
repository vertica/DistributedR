...
*/
int _gethasweights(ifstream &inFile)
{
  char line[MAXLINE];
...
*/
RcppExport SEXP hpdmatrixsplitter (SEXP inputfile, SEXP rows, SEXP cols, SEXP _transpose, SEXP outfile)
{
  BEGIN_RCPP
...
//  bool zerobased = false; // it becomes true when a vertex with id==0 is found
  int hasweights = _gethasweights(inFile); // determines existence of weights on the edges
...
//  Rprintf("%d vertices\n", v);
  ...
  int rowsplitsize = nrow%desired_rowsplits==0 ? nrow/desired_rowsplits : nrow/desired_rowsplits+1;
  int colsplitsize = ncol%desired_colsplits==0 ? ncol/desired_colsplits : ncol/desired_colsplits+1;
  int rowsplits = nrow%rowsplitsize==0 ? nrow/rowsplitsize : nrow/rowsplitsize+1;
  int colsplits = ncol%colsplitsize==0 ? ncol/colsplitsize : ncol/colsplitsize+1;
...
//  memset(lines, 0, splits*sizeof(long));
  ...
  while (inFile)
  {
    ...
    inFile.getline(aline, MAXLINE);
	if(transpose) {
    	fscanner = sscanf(aline, "%d %d %f", &e, &d, &weight);
	} else {
