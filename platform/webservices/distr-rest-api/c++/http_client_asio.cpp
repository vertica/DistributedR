...
public:
    ...
    void start_request()
    {
        ...
        else
        {
            ...
            auto regularptr = shared_from_this();
            auto boostptr = make_shared_ptr<web::http::client::details::asio_context>(regularptr);
            client->m_resolver.async_resolve(query, boost::bind(&asio_context::handle_resolve, boostptr, boost::asio::placeholders::error, boost::asio::placeholders::iterator));
        }
...
private:
    ...
    void handle_connect(const boost::system::error_code& ec, tcp::resolver::iterator endpoints)
    {
        ...
        else
        {
            ...
            auto regularptr = shared_from_this();
            auto boostptr= make_shared_ptr<web::http::client::details::asio_context>(regularptr);
            m_connection->async_connect(endpoint, boost::bind(&asio_context::handle_connect, boostptr, boost::asio::placeholders::error, ++endpoints));
        }
    }
    ...
    void write_request()
    {
        ...
        if (m_connection->is_ssl())
        {
            ...
            m_connection->async_handshake(boost::asio::ssl::stream_base::client,
                                          ...
                                          boost::bind(&asio_context::handle_handshake, boostptr, boost::asio::placeholders::error),
        ...
        {
            m_connection->async_write(m_body_buf, boost::bind(&asio_context::handle_write_headers, boostptr, boost::asio::placeholders::error));
        }
    }
    ...
    void handle_write_chunked_body(const boost::system::error_code& ec)
    {
        ...
        readbuf.getn(buf + http::details::chunked_encoding::data_offset, chunkSize).then([this_request, buf, chunkSize](pplx::task<size_t> op)
        {
            ...
            auto regularptr = this_request;
            auto boostptr= make_shared_ptr<web::http::client::details::asio_context>(regularptr);
            if (readSize != 0)
            {
                this_request->m_connection->async_write(this_request->m_body_buf,
                                                        boost::bind(&asio_context::handle_write_chunked_body, boostptr, boost::asio::placeholders::error));
            }
            else
            {
                this_request->m_connection->async_write(this_request->m_body_buf,
                                                        boost::bind(&asio_context::handle_write_body, boostptr, boost::asio::placeholders::error));
            }
        });
    }
    ...
    void handle_write_large_body(const boost::system::error_code& ec)
    {
        ...
        readbuf.getn(boost::asio::buffer_cast<uint8_t *>(m_body_buf.prepare(readSize)), readSize).then([this_request](pplx::task<size_t> op)
        {
            try
            {
                ...
                auto regularptr = this_request;
                auto boostptr= make_shared_ptr<web::http::client::details::asio_context>(regularptr);
                this_request->m_connection->async_write(this_request->m_body_buf, boost::bind(&asio_context::handle_write_large_body, boostptr, boost::asio::placeholders::error));
            }
            catch (...)
            {
                this_request->report_exception(std::current_exception());
    ...
    void handle_write_body(const boost::system::error_code& ec)
    {
        if (!ec)
        {
            ...
            auto regularptr = shared_from_this();
            auto boostptr= make_shared_ptr<web::http::client::details::asio_context>(regularptr);
            m_connection->async_read_until(m_body_buf, CRLF + CRLF, boost::bind(&asio_context::handle_status_line, boostptr ,boost::asio::placeholders::error));
        }
        else
        {
        ...
        else
        {
            if (!needChunked)
            {
                ...
                async_read_until_buffersize(static_cast<size_t>(std::min(m_content_length, static_cast<uint64_t>(m_http_client->client_config().chunksize()))),
                                            boost::bind(&asio_context::handle_read_content, boostptr, boost::asio::placeholders::error));
            }
            else
            {
                auto regularptr = shared_from_this();
                auto boostptr = make_shared_ptr<web::http::client::details::asio_context>(regularptr);
                m_connection->async_read_until(m_body_buf, CRLF, boost::bind(&asio_context::handle_chunk_header, boostptr , boost::asio::placeholders::error));
            }
        }
    }
        ...
        if (!ec)
        {
            ...
            else
            {
                ...
                async_read_until_buffersize(octets + CRLF.size(), // + 2 for crlf
                                            boost::bind(&asio_context::handle_chunk, boostptr , boost::asio::placeholders::error, octets));
            }
        }
        else
        {
        ...
        if (!ec)
        {
            ...
            else
            {
                ...
                writeBuffer.putn(boost::asio::buffer_cast<const uint8_t *>(m_body_buf.data()), to_read).then([this_request, to_read](pplx::task<size_t> op)
                {
                    ...
                    this_request->m_connection->async_read_until(this_request->m_body_buf, CRLF, boost::bind(&asio_context::handle_chunk_header, boostptr, boost::asio::placeholders::error));
                });
            }
        }
        else
