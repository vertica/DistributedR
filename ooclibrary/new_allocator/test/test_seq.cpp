...
int main(int argc, char* argv[]){
	...
	if (argc < 7){
		fprintf(stderr, "Usage: %s alloc_num test_num object_size(M)\n", argv[0]);
	...
	size_t object_size = atoll(argv[3]) * M;

	printf("Task: %zu allocation of %zuM, %zu sequential access\n", alloc_num, object_size / M, test_num);

	#ifndef USE_ORIGINAL

	size_t object_cache_size = atoll(argv[4]) * M;
	size_t page_cache_size = atoll(argv[5]) * M;
	size_t mmap_object_size = atoll(argv[6]) * 1024UL;
